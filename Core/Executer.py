...
class Executer:
    ...
    def run_test(s_program, s_test):
        ...
        tests, errors, failures, time, coverage, fail_messages = Executer._xml_process(s_program.student_id, s_test.student_id)
        test_result = TestResult(s_program.id, s_test.id, tests, errors, failures, coverage, time)

        return test_result, fail_messages

    @staticmethod
    def _xml_process(student_id_program, student_id_test):
        x_filename = "result-{}".format(student_id_program)
        root = Executer._xml_parser(x_filename)
    ...
    @staticmethod
    def _get_fail_messages(untangle_root):
        msgs = []
