...
class Submission(Base):
    ...
    @staticmethod
    def get_last_submissions_each_user(student_id):
    ...
    @staticmethod
    def get_submission_user(student_id):
        session = Database.session()
        s = session.query(Submission).filter(Submission.student_id == student_id).first()
        session.close()
        return s
